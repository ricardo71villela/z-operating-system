...
try {
  registry = require(path.join(ROOT, 'apps/zfind-web/src/services/market-registry.js'));
} catch (error) {
  console.error('MARKET_REGISTRY_LOAD_ERROR=' + error.message);
}

try {
  routes = require(path.join(ROOT, 'apps/zfind-web/src/services/public-routes.js'));
} catch (error) {
  console.error('PUBLIC_ROUTES_LOAD_ERROR=' + error.message);
}

try {
  generator = require(path.join(ROOT, 'apps/zfind-web/src/services/seo-page-generator.js'));
} catch (error) {
  console.error('SEO_GENERATOR_LOAD_ERROR=' + error.message);
}

try {
  seoScript = require(path.join(ROOT, 'apps/zfind-web/scripts/generate-seo-pages.js'));
} catch (error) {
...
if (registry) {
  ...
  check('every market has labels and slugs in all six locales',
    markets.every(m => expectedLocales.every(locale =>
      typeof m.labels[locale] === 'string' && m.labels[locale].trim() &&
      typeof m.slugs[locale] === 'string' && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(m.slugs[locale])
    )));
  ...
  check('England remains exact constituent-market geography',
    england && england.geography.kind === 'constituent-country' &&
    england.geography.code === 'GB-ENG' && england.geography.parentCountryIso === 'GB' &&
    england.searchScope.kind === 'exact_market' && england.searchScope.value === 'GB-ENG');

  check('Dubai remains Emirate of Dubai, never generic UAE',
    dubai && dubai.geography.kind === 'emirate' && dubai.geography.code === 'AE-DU' &&
    dubai.geography.parentCountryIso === 'AE' && dubai.searchScope.kind === 'exact_market' &&
    dubai.searchScope.value === 'AE-DU' && !markets.some(m => m.key === 'AE'));
  ...
  check('sovereign markets carry explicit country_iso search scopes',
    ['PT','ES','FR','DE','IT','IE','NL','BE','US','CA','MX','BR','AR','CL','DO','PL','GR','HR','CY']
      .every(key => {
        ...
        return market.searchScope.kind === 'country_iso' && market.searchScope.value === key;
      }));

  check('new market presentation copy is complete 6/6 without fallback',
    expectedLocales.every(locale => {
      const p = registry.marketPresentation('PT', locale);
      return [p.heroEyebrow,p.heroTitle,p.heroLead,p.featuredTitle,p.searchTitle,p.guidesTitle,p.legalLabel,p.rentalLabel,p.seoTitle,p.seoDescription]
        .every(value => typeof value === 'string' && value.trim());
    }));

  let rejected = false;
  try { registry.marketPresentation('PT', 'xx'); } catch (_) { rejected = true; }
  check('new market copy rejects unsupported locales instead of fallback', rejected);
}

if (routes) {
  check('public routes expose localized market route segments',
    routes.ROUTES.fr.market === 'marches' && routes.ROUTES.en.market === 'markets' &&
    routes.ROUTES.pt.market === 'mercados' && routes.ROUTES.es.market === 'mercados' &&
    routes.ROUTES.de.market === 'maerkte' && routes.ROUTES.it.market === 'mercati');

  check('clean market path parser recognizes a market URL',
    same(routes.parsePublicPath('/pt/mercados/portugal'), { type:'market', locale:'pt', slug:'portugal' }));
}

check('single reusable market view exists', body.includes('id="view-market"') && body.includes('id="market-root"'));
check('market selectors target marketplace, not legal guide directly',
  (body.match(/data-market-select/g) || []).length === 2 &&
  body.includes('onchange="navigateMarket(this.value)"') && !body.includes('data-market-guide-select'));
check('app dispatches one reusable market renderer',
  app.includes("case 'market': renderMarket(state.id); break;") && app.includes('function renderMarket(marketKey)'));
check('legacy content-navigation-only market guide mode retired',
  !app.includes("MARKET_GUIDE_MODE = 'content-navigation-only'") && !app.includes('const MARKET_GUIDES = Object.freeze'));
check('market selector navigation uses stable market key', app.includes("navigate('market', marketKey, {})"));
...
if (generator && seoScript && registry) {
  check('generic listing and zone SEO presentation is complete 6/6',
    same(generator.LOCALES, expectedLocales));

  check('market SEO locale scope is exact six',
    same(generator.MARKET_LOCALES, expectedLocales));

  const editorialRows = [
    { locale:'fr', title:'Titre réel', description:'Description française réelle.' },
    { locale:'en', title:'Real title', description:'Real English description.' }
  ];
  check('listing SEO never fabricates untranslated locales',
    same(seoScript.genuineEditorialLocales(editorialRows), ['fr','en']) &&
    seoScript.contentForPublicLocale(editorialRows, 'es') === null &&
    seoScript.contentForPublicLocale(editorialRows, 'de') === null &&
