...
check('viewport module exposes provider-neutral synchronization API',
...
check('ordinary bounds normalize without changing semantic orientation',
  JSON.stringify(viewport.normalizeBounds(france)) === JSON.stringify(france));

check('invalid latitude ordering and invalid coordinates fail closed',
...
check('ordinary viewport contains Paris and excludes Porto',
...
check('antimeridian-crossing bounds include both sides and exclude Greenwich',
...
check('filter preserves source order and source pin references',
  visibleFrance.length === 2 && visibleFrance[0] === pins[0] && visibleFrance[1] === pins[1]);
...
check('computed European bounds cover every valid pin and ignore invalid coordinates',
  europeBounds &&
  ...
  europeBounds.south === 41.1579 && europeBounds.north === 50.62925,
  europeBounds);
...
check('computed bounds choose the short antimeridian-crossing longitude arc',
  datelineBounds && datelineBounds.west === 179 && datelineBounds.east === -179 &&
  viewport.pointInBounds(10, 179, datelineBounds) &&
  viewport.pointInBounds(-10, -179, datelineBounds) &&
  !viewport.pointInBounds(0, 0, datelineBounds),
  datelineBounds);

const single = viewport.computeBoundsForPins([{ id: 'single', latitude: 43.6047, longitude: 1.4442 }]);
check('single-pin bounds collapse exactly to the authored point',
  single && single.south === 43.6047 && single.north === 43.6047 &&
  single.west === 1.4442 && single.east === 1.4442,
  single);

check('no valid coordinates produce no synthetic bounds',
...
check('viewport state synchronizes visible list ids and visible selection',
  state.totalPinCount === 4 && state.visiblePinCount === 2 &&
  state.visibleIds.join(',') === 'paris,lille' && state.selectedId === 'lille' &&
  state.visiblePins[1] === pins[1], state);

const hiddenSelection = viewport.buildViewportState(pins, france, 'porto');
check('selection outside current viewport fails closed without inventing visibility',
  hiddenSelection.selectedId === null && hiddenSelection.visibleIds.join(',') === 'paris,lille');

const invalidState = viewport.buildViewportState(pins, { south: 20, west: 0, north: 10, east: 1 }, 'paris');
check('invalid viewport yields explicit empty synchronized state',
  invalidState.bounds === null && invalidState.visiblePinCount === 0 &&
  invalidState.visibleIds.length === 0 && invalidState.selectedId === null);

check('viewport foundation performs no executable network, geocoding or provider SDK call',
