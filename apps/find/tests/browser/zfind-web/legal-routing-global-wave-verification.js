...
const countries = [
  { iso:'CL', label:'Chile', legal:'legal-chile', tourist:'tourist-rental-chile', legalMarker:'Boletín 18.216-05', touristMarker:'Ley 21.442' },
  { iso:'DO', label:'República Dominicana', legal:'legal-dominican-republic', tourist:'tourist-rental-dominican-republic', legalMarker:'Ley 30-26', touristMarker:'RENATUR' },
  { iso:'PL', label:'Poland', legal:'legal-poland', tourist:'tourist-rental-poland', legalMarker:'PIT-39', touristMarker:'CWTON' },
  { iso:'GR', label:'Greece', legal:'legal-greece', tourist:'tourist-rental-greece', legalMarker:'€800,000', touristMarker:'Short-Term Stay Property Registry' },
  { iso:'HR', label:'Croatia', legal:'legal-croatia', tourist:'tourist-rental-croatia', legalMarker:'Croatian citizens under 45', touristMarker:'early 2027' },
  { iso:'CY', label:'Cyprus', legal:'legal-cyprus', tourist:'tourist-rental-cyprus', legalMarker:'Law 239(I)/2025', touristMarker:'Special Label and Registration Number' },
  { iso:'AE-DU', label:'Dubai', legal:'legal-dubai', tourist:'tourist-rental-dubai', legalMarker:'Seller: 2%', touristMarker:'Holiday Home permit' },
];

const translatedLocales = ['fr', 'en', 'pt', 'es', 'de', 'it'];

let passed = 0;
let failed = 0;

async function launch() {
  const executablePath = process.env.LOCAL_SANDBOX_CHROMIUM_PATH;
...
(async () => {
  ...
  async function open(locale, route) {
    await page.goto(`${URL}#/${locale}/${route}`, { waitUntil:'load' });
    await page.waitForFunction(requestedRoute => {
      const active = document.querySelectorAll('.view.active');
      return active.length === 1 && active[0].id === `view-${requestedRoute}`;
    }, route);
  }
  ...
  async function requireDisclaimer(route) {
    const text = await page.locator(`#view-${route} .disclaimer`).innerText();
    const valid =
      text.includes('qualified local legal professional') ||
      text.includes('profesional local cualificado') ||
      text.includes('profissional local qualificado') ||
      text.includes('profissional habilitado');
    if (!valid) throw new Error('jurisdiction-specific disclaimer absent');
  }
  ...
  for (const country of countries) {
    ...
    await check(`${country.label} legal content marker is visible`, async () => {
      await open('en', country.legal);
      const text = await page.locator(`#view-${country.legal}`).innerText();
      if (!text.includes(country.legalMarker)) throw new Error(`legal marker absent: ${country.legalMarker}`);
    });

    await check(`${country.label} tourist content marker is visible`, async () => {
      await open('en', country.tourist);
      const text = await page.locator(`#view-${country.tourist}`).innerText();
      if (!text.includes(country.touristMarker)) throw new Error(`tourist marker absent: ${country.touristMarker}`);
    });

    await check(`${country.label} legal disclaimer remains visible`, async () => {
      await open('en', country.legal);
      await requireDisclaimer(country.legal);
    });

    await check(`${country.label} tourist disclaimer remains visible`, async () => {
      await open('en', country.tourist);
  ...
  for (const locale of translatedLocales) {
    for (const country of countries) {
      ...
      await check(`${country.label} tourist route remains ${country.label} under UI locale ${locale}`, () => open(locale, country.tourist));
    }
  }
  ...
  await check('Dubai legal guide is explicitly Emirate of Dubai only', async () => {
    ...
    const text = await page.locator('#view-legal-dubai').innerText();
    if (!text.includes('Emirate of Dubai only')) throw new Error('Dubai legal scope is not explicit');
  });

  await check('Dubai tourist guide is explicitly scoped to the Emirate of Dubai', async () => {
    await open('en', 'tourist-rental-dubai');
    const text = await page.locator('#view-tourist-rental-dubai').innerText();
    if (!text.includes('Emirate of Dubai')) throw new Error('Dubai tourist scope is not explicit');
  });

  await check('Phase-4 public locale authority remains six-language', async () => {
    await open('fr', 'legal-chile');
    const actual = await page.evaluate(() => Array.from(window.ZFindServices.publicLocales.PUBLIC_LOCALES));
    if (JSON.stringify(actual) !== JSON.stringify(translatedLocales)) throw new Error('unexpected Phase-4 PUBLIC_LOCALES');
  });

  await check('compact language menu exposes six translated and selectable Phase-4 languages', async () => {
    await open('fr', 'legal-chile');
    ...
    const expectedMenu = translatedLocales.map(lang => ({ lang, disabled:false }));
    if (JSON.stringify(actual.translated) !== JSON.stringify(translatedLocales)) throw new Error('translated locale authority drift');
    if (JSON.stringify(actual.menu) !== JSON.stringify(expectedMenu)) throw new Error('compact six-language menu contract drift');
    if (actual.current !== 'FR') throw new Error('French default compact-menu label drift');
  });
  ...
  if (pageErrors.length) {
    ...
    for (const error of pageErrors) console.error(`PAGE_ERROR: ${error}`);
  }
  ...
  if (failed || passed !== expectedPassed) {
    console.error('GLOBAL LEGAL WAVE DISTINCT JURISDICTION CONTRACT: ' + `${passed} PASSED, ${failed} FAILED`);
    process.exit(1);
  }

  console.log('GLOBAL LEGAL WAVE DISTINCT JURISDICTION CONTRACT: ' + `${expectedPassed}/${expectedPassed} PASSED`);
})().catch(error => {
  console.error(error);
  process.exit(1);
});
