...
(function (root, factory) {
  if (typeof module === 'object' && module.exports) {
    ...
    root.ZFindServices.searchMapInteraction = factory(root.ZFindServices.searchMapViewport);
  }
  ...
  function normalizeBounds(bounds) {
    return viewport && typeof viewport.normalizeBounds === 'function' ? viewport.normalizeBounds(bounds) : null;
  }
  ...
  function copyBounds(bounds) {
    return bounds ? { south: bounds.south, west: bounds.west, north: bounds.north, east: bounds.east } : null;
  }
  ...
  function freezeState(state) {
    const frozen = {
      ...
      intent: state.intent ? Object.freeze({
        ...state.intent,
        bounds: copyBounds(state.intent.bounds),
        memberIds: state.intent.memberIds ? Object.freeze([...state.intent.memberIds]) : undefined
      }) : null
    };
  ...
  function createInteractionState(options) {
    ...
    return freezeState({
      ...
      committedBounds,
      visibleIds,
      selectedId: requestedSelectedId && visibleIds.includes(requestedSelectedId) ? requestedSelectedId : null,
      highlightedId: requestedHighlightedId && visibleIds.includes(requestedHighlightedId) ? requestedHighlightedId : null,
  ...
  function selectListing(state, listingId, origin) {
    ...
    if (!id || !selectionOrigin || !current.visibleIds.includes(id)) return current;
    return freezeState({
      ...current,
      selectedId: id,
      activeClusterId: null,
      ...
      intent: { type: selectionOrigin === 'pin' ? 'reveal-card' : 'reveal-pin', id, origin: selectionOrigin }
    });
  }
  function highlightListing(state, listingId, origin) {
    ...
    if (!id || !highlightOrigin || !current.visibleIds.includes(id)) return current;
    return freezeState({ ...current, highlightedId: id, lastOrigin: highlightOrigin, intent: null });
  }
  ...
  function selectCluster(state, cluster) {
    ...
    const members = normalizeIdList(cluster.memberIds)
      .filter(id => current.visibleIds.includes(id)).sort();
    ...
    return freezeState({
      ...
      lastOrigin: 'cluster',
      intent: { type: 'fit-cluster', clusterId, memberIds: members }
    });
  }
  ...
  function commitSearchArea(state) {
    ...
    return freezeState({
      ...
      intent: { type: 'run-search-area', bounds: viewportBounds }
    });
  }
  function syncVisibleResults(state, visibleIds) {
    const current = state || createInteractionState();
    const nextVisibleIds = normalizeIdList(visibleIds);
    const selectedId = current.selectedId && nextVisibleIds.includes(current.selectedId) ? current.selectedId : null;
    const highlightedId = current.highlightedId && nextVisibleIds.includes(current.highlightedId) ? current.highlightedId : null;
    const clusterMemberIds = current.clusterMemberIds.filter(id => nextVisibleIds.includes(id));
    const activeClusterId = clusterMemberIds.length >= 2 ? current.activeClusterId : null;
    return freezeState({
      ...current,
  ...
  return Object.freeze({
    SELECTION_ORIGINS, VIEWPORT_CAUSES, boundsSignature, sameBounds,
    createInteractionState, selectListing, highlightListing, clearHighlight,
    selectCluster, changeViewport, commitSearchArea, syncVisibleResults, clearIntent
  });
});
