...
(function (root, factory) {
  ...
  function pointInBounds(latitude, longitude, bounds) {
    ...
    // Antimeridian crossing, e.g. west=170, east=-170.
    return lon >= normalized.west || lon <= normalized.east;
  }
  ...
  function filterPinsInBounds(pins, bounds) {
    ...
    return pins.filter(pin =>
      pin && pointInBounds(pin.latitude, pin.longitude, normalized)
    );
  }
  ...
  function buildViewportState(pins, bounds, selectedId) {
    ...
    const visiblePins = normalized
      ? filterPinsInBounds(sourcePins, normalized)
      : [];
    ...
    const visibleSelectedId = requestedSelection && visibleIds.includes(requestedSelection)
      ? requestedSelection
      : null;
