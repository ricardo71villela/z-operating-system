...
export function PersonnelTabs({ locale, labels }: Props) {
  ...
  useEffect(() => {
    ...
    let url = "";
    if (tab === "monthly") {
      url = `/api/desk/personnel/monthly-map?year=${now.getFullYear()}&month=${now.getMonth() + 1}`;
    } else if (tab === "weekly") {
      const weekStart = mondayOf(now).toISOString().slice(0, 10);
      url = `/api/desk/personnel/weekly-view?weekStart=${weekStart}`;
    } else {
      const weekStart = mondayOf(now).toISOString().slice(0, 10);
      url = `/api/desk/personnel/workload-map?weekStart=${weekStart}`;
    }
