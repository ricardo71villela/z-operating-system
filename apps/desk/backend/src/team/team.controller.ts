import { Body, Controller, Delete, ForbiddenException, Get, Headers, Param, Patch, Post, Req, UnauthorizedException } from '@nestjs/common';
...
interface MembershipRow { id: string; person_id: string; }
interface PersonRow { id: string; display_name: string | null; }
...
export class TeamController {
  @Get('context')
  @RequireDeskAuth()
  context(@Req() req: DeskRequest) {
    const context = req.deskContext!;
    return { workspaceMemberId: context.workspaceMemberId, role: context.role };
  }
  ...
  async listMembers(@Req() req: DeskRequest) {
    ...
    const { data: memberships, error: membershipsError } = await zosAdmin.from('memberships').select('id,person_id').in('id', membershipIds);
    if (membershipsError) throw membershipsError;
    const membershipRows = (memberships ?? []) as MembershipRow[];
    const personIds = membershipRows.map((membership) => membership.person_id);
    const { data: persons, error: personsError } = await zosAdmin.from('persons').select('id,display_name').in('id', personIds);
    ...
    return members.map((member) => {
      ...
      return {
        ...
        displayName: person?.display_name ?? 'Z Desk member',
        role: member.role,
        status: member.status,
        preferredLanguage: member.preferred_language,
        createdAt: member.created_at,
        current: member.id === context.workspaceMemberId,
      };
    });
  }
  ...
  async listInvitations(@Req() req: DeskRequest) {
    const context = req.deskContext!;
    const { data, error } = await deskAdmin
      .from('workspace_invitations')
      .select('id,invited_email,role,status,expires_at,created_at,accepted_at')
      .eq('workspace_id', context.workspaceId)
      .order('created_at', { ascending: false });
    if (error) throw error;
    const now = Date.now();
    return (data ?? [])
      .filter((invitation) => context.role === 'owner' || invitation.role === 'member')
      .map((invitation) => ({
        invitationId: invitation.id,
        email: invitation.invited_email,
        role: invitation.role,
        status: invitation.status === 'pending' && new Date(invitation.expires_at).getTime() <= now ? 'expired' : invitation.status,
  ...
  @RequireDeskAuth()
  @RequireDeskRoles('owner', 'admin')
  async createInvitation(@Req() req: DeskRequest, @Body() body: { email?: string; role?: InviteRole }) {
    const context = req.deskContext!;
    ...
    if (context.role === 'admin' && role !== 'member') throw new ForbiddenException('Desk admins may invite members only.');
    ...
    if (error) throw error;
    return { ...(data as Record<string, unknown>), token: secret.token, delivery: 'secure_link_ready' };
  }

  @Post('invitations/:id/reissue')
  @RequireDeskAuth()
  @RequireDeskRoles('owner', 'admin')
  async reissueInvitation(@Req() req: DeskRequest, @Param('id') id: string) {
    const context = req.deskContext!;
    const secret = newInvitationSecret();
    const { data, error } = await supabaseAdmin.rpc('zdesk_reissue_invitation', {
      p_workspace_id: context.workspaceId,
  ...
  @RequireDeskAuth()
  @RequireDeskRoles('owner', 'admin')
  async revokeInvitation(@Req() req: DeskRequest, @Param('id') id: string) {
    const context = req.deskContext!;
    const { data, error } = await supabaseAdmin.rpc('zdesk_revoke_invitation', {
      p_workspace_id: context.workspaceId,
      p_actor_member_id: context.workspaceMemberId,
      p_invitation_id: id,
    });
  ...
  async acceptInvitation(@Headers('authorization') authHeader: string, @Body() body: { token?: string }) {
  ...
  async setMemberRole(@Req() req: DeskRequest, @Param('memberId') memberId: string, @Body() body: { role?: InviteRole }) {
    const context = req.deskContext!;
    const role = normalizeInviteRole(body.role);
