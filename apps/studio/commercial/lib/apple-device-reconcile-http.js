...
export class StudioAuthBoundaryError extends Error {
  constructor(code, { invalid = false, cause } = {}) {
    ...
    this.name = 'StudioAuthBoundaryError';
    this.code = code;
...
function jwsFromBody(body) {
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    throw new Error('APPLE_RECONCILE_REQUEST_INVALID');
  }
  if (Object.keys(body).sort().join('\n') !== 'jwsRepresentation') {
    throw new Error('APPLE_RECONCILE_REQUEST_INVALID');
  }
  const jws = String(body.jwsRepresentation ?? '').trim();
  if (!jws || jws.length > 65536) {
    throw new Error('APPLE_RECONCILE_JWS_INVALID');
  }
  return jws;
}
...
function retryableAppleError(error) {
  if (Number.isInteger(error?.httpStatusCode)) {
    return error.httpStatusCode === 408
...
export async function validateSupabaseBearerAndResolvePerson(
  config,
  token,
  ...
  if (!/^https:\/\//i.test(url) || !publishableKey || !token) {
    throw new StudioAuthBoundaryError('STUDIO_AUTH_CONFIG_INVALID');
  }
  ...
  const call = async (endpoint, options) => {
    ...
    try {
      ...
      throw new StudioAuthBoundaryError('STUDIO_AUTH_UNAVAILABLE', { cause });
    } finally {
      clearTimeout(timer);
    }
  };
  ...
  if (!user.ok) {
    if (user.status === 401 || user.status === 403) {
      throw new StudioAuthBoundaryError('STUDIO_AUTH_INVALID', { invalid: true });
    }
  ...
  try {
    ...
    throw new StudioAuthBoundaryError('STUDIO_ACCOUNT_RESPONSE_INVALID', { cause });
  }
  if (typeof personId !== 'string' || !UUID_PATTERN.test(personId)) {
    throw new StudioAuthBoundaryError('STUDIO_ACCOUNT_RESPONSE_INVALID');
  }
...
export function createAppleDeviceReconcileHttpHandler({
  ...
  if (
    ...
    || typeof reconcileCurrentState !== 'function'
    || typeof applyCommercialEvent !== 'function'
  ) {
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      sendJson(res, 400, { code: 'INVALID_JSON', request_id: requestId }, origin, origins);
      return;
    }

    let jws;
    try {
      jws = jwsFromBody(body);
    } catch (error) {
    ...
    try {
      ...
      sendJson(res, 503, { code: 'AUTH_UNAVAILABLE', request_id: requestId }, origin, origins);
      return;
    }

    let deviceEvidence;
    try {
      deviceEvidence = await verifyTransaction(jws, config);
    } catch {
      sendJson(res, 422, { code: 'APPLE_EVIDENCE_REJECTED', request_id: requestId }, origin, origins);
      return;
    }

    if (deviceEvidence.appAccountToken !== personId) {
      sendJson(res, 403, { code: 'APPLE_PURCHASE_IDENTITY_MISMATCH', request_id: requestId }, origin, origins);
      return;
    }

    let snapshot;
    try {
      snapshot = await reconcileCurrentState(deviceEvidence, config);
    } catch (error) {
      const retryable = retryableAppleError(error);
      sendJson(
        res,
        retryable ? 503 : 422,
        { code: retryable ? 'APPLE_RECONCILIATION_UNAVAILABLE' : 'APPLE_RECONCILIATION_REJECTED', request_id: requestId },
        origin,
        origins,
    ...
    let written;
    try {
      written = await applyCommercialEvent(snapshot, config);
    } catch (error) {
      if (error instanceof CommercialWriterRpcError && error.retryable) {
        sendJson(res, 503, { code: 'COMMERCIAL_WRITE_UNAVAILABLE', request_id: requestId }, origin, origins, { 'Retry-After': '5' });
        return;
      }
      if (error instanceof CommercialWriterRpcError && error.databaseCode) {
    ...
    sendJson(res, 200, {
      ...
      studio_access_status: written.studioAccessStatus,
      ai_access_status: written.aiAccessStatus,
